    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <form action="changepasswordadmin.php" method="POST">
              ...
              <div>
                <button type="submit" name="submited" class="btn btn-success btn-sm">Change password</button>
...
// Designed by Vaishak Murali.
  ...
  if($_SESSION['type']=="emp"){
    ...
    if(empty($name)){
      ...
      <strong>Error : Invalid Input given. Enter patient name </strong></center></div>";
      die();
    }
    $encypass = md5(md5($name).$password);
    mysqli_query($conn,"UPDATE login SET password='".$encypass."' WHERE useername='".$name."' AND usertype='".$usertype."'");

    if (mysqli_affected_rows($conn)==1) {
      echo "<div class='alert alert-success'><center>
      <strong>Password changed!</strong></center></div>";
    }
